...
function Stars({ score }: { score: number | null }) {
  if (score == null) return <Text className="text-slate-400 text-xs">—</Text>;
  return (
  ...
  if (sessions.length === 0) {
    return (
      <View className="rounded-2xl border border-dashed border-slate-200 bg-slate-50 p-6">
        <Text className="text-slate-900">No sessions logged yet</Text>
    ...
    <View>
      {sessions.map((session, index) => (
        <View key={session.id} className="flex-row mb-4">
          ...
          <View className="flex-1 bg-white border border-slate-100 rounded-xl p-3">
            <View className="flex-row justify-between items-start">
              <View className="flex-1 pr-2">
                <Text className="text-[10px] uppercase tracking-widest text-slate-400">
                  {session.activityType === "HIFZ" ? "Hifz" : "Muraja"}
                </Text>
                <Text className="text-slate-900 text-sm mt-0.5">{session.reference}</Text>
                <Text className="text-slate-500 text-xs mt-0.5">{session.planName}</Text>
              </View>
              {session.isMissed ? (
            ...
            <View className="flex-row flex-wrap gap-3 mt-2">
              <Text className="text-slate-500 text-xs">{formatDate(session.date)}</Text>
              <Text className="text-slate-500 text-xs">{session.durationMinutes} min</Text>
              <Text className="text-slate-500 text-xs">{session.pagesCompleted} pgs</Text>
      ...
      {hasMore ? (
        <Pressable
          ...
          className="flex-row items-center justify-center py-3 border border-slate-200 rounded-xl"
        >
