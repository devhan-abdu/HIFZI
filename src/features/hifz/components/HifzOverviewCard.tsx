...
export default function HifzOverViewCard({
  ...
  const labelColor = isWhite ? "text-muted" : "text-white/50";
  const mainTextColor = isWhite ? "text-text" : "text-white";
  const badgeBg = isWhite ? "bg-surface" : "bg-surface/20";
  const badgeText = isWhite ? "text-muted" : "text-white";
  const badgeBorder = isWhite ? "border-border" : "border-white/10";
  ...
  return (
    ...
    >
      ...
      <View className="absolute inset-0 items-center justify-center px-4">
        ...
        <View
          className={`h-[1px] w-8 my-1.5 ${isWhite ? "bg-surface" : "bg-surface/20"}`}
        />
